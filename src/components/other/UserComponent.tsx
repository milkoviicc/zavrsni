  ...
  return (
    <div className='w-fit'>
      <div className="hover:cursor-pointer hidden gap-2 py-2 items-center lg:flex" onClick={() => routeToUser()}>
          <Avatar className='w-[50px] h-[50px] 2k:w-[65px] 2k:h-[65px] rounded-full'>
          ...
          <div className="flex flex-col h-full items-start justify-center lg:w-[95px] xl:w-[100px] 2xl:max-w-[150px]">
              <h1 className="text-[#EFEFEF] font-[400] font-Roboto text-base 2k:text-lg lg:max-w-[95px] xl:max-w-[100px] 2xl:max-w-[150px] truncate whitespace-nowrap" title={`${user?.firstName} ${user?.lastName}`}>{user?.firstName} {user?.lastName}</h1>
              <p className="text-[#888888] lg:max-w-[95px] xl:max-w-[100px] 2xl:max-w-[150px] text-base 2k:text-lg truncate whitespace-nowrap">@{user?.username}</p>
          </div>    
      </div>
