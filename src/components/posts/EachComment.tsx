...
const EachComment = ({post, comment, refreshComments, updateComment, callComments}: {post: Post, comment: Comment, refreshComments: () => void, updateComment: (commentId: string, newContent: string) => void, callComments: React.Dispatch<React.SetStateAction<boolean>>})=> {
    ...
    const handleCommentLike = async (commentId: string) => {
      try {
          ...
          if(comment.userReacted === -1) {
            await reactionsApi.updateCommentReaction(commentId);
          }
          ...
          if (comment.userReacted === 0) {
            await reactionsApi.addCommentReaction(commentId, 1);
          }
    ...
    const deleteReply = async (replyId: string) => {
      try {
        // šaljem axios delete request na API sa id-em posta i spremam response u varijablu 'res'
        const res = await commentsApi.deleteComment(replyId);
    ...
    const update = async () => {
      if(comment.content === updatedContent) {
        toast("You must change text to update your comment", {duration: 1500, style:{backgroundColor: "#CA3C3C", border: "none", color: "#fff"}});
        return;
      }
    ...
    const handleReply = async (replyContent: string) => {
      if(replyContent === '') {
        toast("Text is required!", {duration: 1500, style: {backgroundColor: "#CA3C3C", border: "none", color: "#fff"}});
        return;
      }
      try {
          ...
          if(res.status === 200) {
            ...
            toast("Reply successfully posted.", {duration: 1500, style: {backgroundColor: "#1565CE", border: "none", color: "#fff"}});
            callComments(true);
          }
          setReplyContent('');
    ...
    const handleReplyLike = async (commentReplyId: string) => {
      try {
        ...
        if(reply.userReacted === 1) {
          await reactionsApi.deleteCommentReaction(commentReplyId);
        }

        // ukoliko je trenutan post dislikean (-1) mjenja se iz dislike u like axios put requestom na API
        if(reply.userReacted === -1) {
          await reactionsApi.updateCommentReaction(commentReplyId);
        }

        // ukoliko trenutan post nije ni likean ni dislikean, šalje se axios post request na API sa query vrijednošću 1 kako bi se post likeao
        if (reply.userReacted === 0) {
          await reactionsApi.addCommentReaction(commentReplyId, 1);
        }
              ...
              <div>
                {showUpdate && (
                  <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                      <PopoverTrigger><EllipsisIcon className="text-[#AFAFAF] size-7 mx-2 cursor-pointer" /></PopoverTrigger>
