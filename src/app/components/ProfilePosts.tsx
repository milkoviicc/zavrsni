...
const ProfilePosts = () => {
    ...
    const [postsState, setPostsState] = useState<'Someones' | 'My profile'>();
    ...
    useEffect(() => {
        const someones = pathname.includes('/users/');
        const myProfile = pathname === '/my-profile';
        if(someones) {
            setPostsState('Someones');
            setNotUser(true);
        } else if(myProfile) {
    ...
    const getPosts = async (page: number) => {
        try {
            if(notUser) {
                ...
                const res = await axios.get(`https://snetapi-evgqgtdcc0b6a2e9.germanywestcentral-01.azurewebsites.net/api/posts/user/${username}`);

                if(res.status === 200) {
                    setPosts(res.data);
                }
            } else {
                const res = await axios.get(`https://snetapi-evgqgtdcc0b6a2e9.germanywestcentral-01.azurewebsites.net/api/posts/user/${userData?.userId}`);
    ...
    const getPostsQuery = useQuery({queryKey: ["getPosts"], queryFn: () => getPosts(currentPage), enabled: userData !== undefined || notUser});
    ...
    <div className="border-1 border-gray-900 h-full flex flex-col items-center gap-4 w-full 2xl:py-12 py-0">
            ...
            <div className="flex flex-col w-[85%] relative mt-6 py-2 px-4 shadow-[1px_3px_4px_0px_rgba(0,_0,_0,_0.3)] bg-[#363636] rounded-full">
                <div className='w-full flex justify-between items-center gap-2' onClick={() => setPostDialogOpen(true)}>
                    <Avatar className='w-[32px] h-[32px] rounded-full'>
                        <AvatarImage src={`${userData?.pictureUrl}`} className="w-fit h-fit aspect-square rounded-full object-cover" style={{boxShadow: '0px 6px 6px 0px #00000040'}} />
                    </Avatar>
                    <div className="flex flex-col w-full">
                    <div className='flex justify-between items-center w-full'>
                        <textarea value={`What's on your mind, ${userData?.firstName}`} readOnly onClick={() => setPostDialogOpen(true)} className="resize-none truncate whitespace-nowrap font-Roboto font-normal scrollbar-none h-[20px] md:min-w-[310px] w-full md:w-full pr-2 text-sm text-[#fff] outline-none rounded border-gray-800 hover:border-gray-600 focus:border-gray-600 placeholder-[#BBBBBB] bg-transparent transition-all"/>
                          ...
                          <div className="flex flex-col justify-between relative w-full min-h-fit items-center gap-4 pt-4 px-4">
                              <div className='w-full h-full flex gap-4 pb-2'>
                                  ...
                                  <div className='flex flex-col flex-grow gap-4'>  
                                    <ResizableTextarea onChange={(e) => setContent(e.target.value)} value={content} placeholder={`What's on your mind, ${userData?.firstName}`} className="font-Roboto font-normal leading-5 scrollbar-none w-full max-h-[100px] lg:max-h-[150px] text-sm lg:text-lg text-[#EFEFEF] outline-none rounded border-gray-800 hover:border-gray-600 focus:border-gray-600 placeholder-[#BBBBBB] bg-transparent transition-all"/>
          ...
          <div className="flex gap-2 items-center flex-col max-w-full rounded-3xl bg-[#363636] shadow-[1px_3px_4px_0px_rgba(0,_0,_0,_0.3)]">
              <div className="flex flex-col justify-between relative w-full min-h-fit items-center gap-4 pt-4 px-4">
                  <div className='w-full h-full flex gap-4 pb-2 '>
                      ...
                      <div className='flex flex-col flex-grow gap-4'>  
                        <ResizableTextarea onChange={(e) => setContent(e.target.value)} value={content} placeholder={`What's on your mind, ${userData?.firstName}`} className="font-Roboto font-normal leading-5 scrollbar-none w-[500px] max-h-[100px] lg:max-h-[150px] text-sm md:text-lg text-[#EFEFEF] outline-none rounded border-gray-800 hover:border-gray-600 focus:border-gray-600 placeholder-[#BBBBBB] bg-transparent transition-all"/>
                        <div className='flex justify-end gap-4 items-center'>
                            <div className='flex h-full justify-center items-center'>
        ...
        <div className="h-full w-full sm:flex hidden flex-col items-center">
            <div className="flex gap-4 py-6 items-center">
              <h1>{user ? 'Your posts' : 'Their posts'}</h1>
            </div>
